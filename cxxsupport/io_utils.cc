...
void readPointing (iohandle &inp, int ctheta, int cphi, int cpsi,
  ...
  while (cm.getNext(start,size))
    {
    inp.readColumnRaw(ctheta,&psi[start],size,start+offset);
    for (tsize i=0; i<size; ++i) pt[start+i].theta=psi[start+i];
    inp.readColumnRaw(cphi,&psi[start],size,start+offset);
    for (tsize i=0; i<size; ++i) pt[start+i].phi=psi[start+i];
    inp.readColumnRaw(cpsi,&psi[start],size,start+offset);
    }
  }
