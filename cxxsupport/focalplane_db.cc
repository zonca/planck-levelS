...
void focalplane_db::init(const string &fname, bool strict_type)
  {
  handle = strict_type ?
             HandleManager.openObject(fname,"focalplane.LS_focalplanedb")
           : HandleManager.openObject(fname);
...
//FIXME: temporary
focalplane_db::focalplane_db (const paramfile &parfile, bool strict_type)
  {
  string fname = parfile.find<string>("focalplane_db");
  init(fname, strict_type);
  }

focalplane_db::focalplane_db (const string &fname, bool strict_type)
  {
  init(fname, strict_type);
  }
