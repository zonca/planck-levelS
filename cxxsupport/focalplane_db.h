...
class focalplane_db
  ...
  private:
    ...
    void init(const std::string &fname, bool strict_type);

  public:
    /*! Reads the focal plane database from the file specified under
        \c focalplane_db in \a parfile.
        \note When used with HFIDMC, \a strict_type must be \c true! */
    focalplane_db (const paramfile &parfile, bool strict_type=true);
    /*! \note When used with HFIDMC, \a strict_type must be \c true! */
    focalplane_db (const std::string &fname, bool strict_type=true);
    ~focalplane_db();
