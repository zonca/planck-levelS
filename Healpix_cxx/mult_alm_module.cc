...
template<typename T> void mult_alm (paramfile &params)
  ...
  if (!polarisation)
    {
    int nlmax, nmmax;
    get_almsize_dmc(infile, nlmax, nmmax);
    Alm<xcomplex<T> > alm;
    read_Alm_from_dmc(infile,alm,nlmax,nmmax);
    ...
    if (cl_in!="")
      {
      read_powspec_from_dmc (cl_in,tps,1,alm.Lmax());
      for (int l=0; l<=nlmax; ++l)
    ...
    if (cl_out!="")
      {
      read_powspec_from_dmc (cl_out,tps,1,alm.Lmax());
      for (int l=0; l<=nlmax; ++l)
    ...
    write_Alm_to_dmc (outfile,alm,nlmax,nmmax);
    }
  else
    {
    int nlmax, nmmax;
    get_almsize_dmc_pol(infile, nlmax, nmmax);
    ...
    write_Alm_to_dmc (outfile,almT,almG,almC,nlmax,nmmax);
    }
  }
