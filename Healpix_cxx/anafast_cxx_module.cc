...
template<typename T> void anafast_cxx (paramfile &params)
  ...
  if (!polarisation)
    ...
    if (outfile!="")
      ...
      write_powspec_to_dmc (outfile,powspec,1);
      }
    if (outfile_alms!="")
      write_Alm_to_dmc (outfile_alms,alm,nlmax,nmmax);
    }
  else
    {
    ...
    if (outfile!="")
      ...
      params.template find<bool>("full_powerspectrum",false) ?
        write_powspec_to_dmc (outfile,powspec,6) :
        write_powspec_to_dmc (outfile,powspec,4);
      }
    if (outfile_alms!="")
      write_Alm_to_dmc (outfile_alms,almT,almG,almC,nlmax,nmmax);
    }
  }
