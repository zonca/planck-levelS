...
int calc_powspec_module (int argc, const char **argv)
  ...
  if (!params.param_present("alm2"))
    {
    int lmax,mmax;
    pol ? get_almsize_dmc_pol(alm1,lmax,mmax)
        : get_almsize_dmc    (alm1,lmax,mmax);
    Alm<xcomplex<float> > almT, almG, almC;
    pol ? read_Alm_from_dmc (alm1,almT,almG,almC,lmax,mmax)
        : read_Alm_from_dmc (alm1,almT,lmax,mmax);
    PowSpec powspec;
    pol ? extract_powspec (almT,almG,almC,powspec)
        : extract_powspec (almT,powspec);
    write_powspec_to_dmc (ps,powspec,pol ? 6 : 1);
    }
  else
    {
    ...
    read_Alm_from_dmc (alm1,Alm1,lmax,mmax);
    string alm2=params.find<string>("alm2");
    get_almsize_dmc(alm2,lmax,mmax);
    Alm<xcomplex<float> > Alm2;
    read_Alm_from_dmc (alm2,Alm2,lmax,mmax);
    PowSpec powspec;
    extract_crosspowspec (Alm1,Alm2,powspec);
    write_powspec_to_dmc (ps,powspec,1);
    }
