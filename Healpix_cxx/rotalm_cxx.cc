...
PLANCK_DIAGNOSIS_BEGIN
  ...
  if (!polarisation)
    {
    int lmax, dummy;
    get_almsize_dmc (infile,lmax,dummy);
    read_Alm_from_dmc (infile, almT, lmax, lmax);
    rotate_alm(almT,rm);
    write_Alm_to_dmc (outfile,almT,lmax,lmax);
    }
  else
    {
    int lmax, dummy;
    get_almsize_dmc_pol (infile,lmax,dummy);
    read_Alm_from_dmc (infile, almT, almG, almC, lmax, lmax);
    rotate_alm(almT,almG,almC,rm);
    write_Alm_to_dmc (outfile,almT,almG,almC,lmax,lmax);
    }
