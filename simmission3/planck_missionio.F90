...
contains
  ...
  function gn_eclang2ang(eclang) result(ang)
    ...
    call gn_assert(eclang%lambda>=0.0, &
      'gn_eclang2ang: lambda < 0.0', eclang%lambda)
    call gn_assert(eclang%lambda<360.0, &
      'gn_eclang2ang: lambda >= 360.0', eclang%lambda)

    ! The latitude, beta, must be between - 90.0 and 90.0.
    call gn_assert(eclang%beta>=-90.0, &
      'gn_eclang2ang: beta < -90.0', eclang%beta)
    call gn_assert(eclang%beta<=90.0, &
      'gn_eclang2ang: beta > 90.0', eclang%beta)
  ...
  subroutine pl_readpointings_nom(filename, dataformat, &
    t_start, t_end, n_pt, zaxis_pts, t_startpts, period_pts)
    character(len=*), intent(in) :: filename, dataformat
    type(gnsec), intent(out) :: t_start, t_end
...
! Fix period durations in case of "plan" format
    ...
    t_start = t_startpts(1)
    t_end = t_startpts(n_pt) + period_pts(n_pt)
