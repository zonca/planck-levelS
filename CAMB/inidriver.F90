...
contains
    ...
    function camb_module (init_object)
      use planck_config

      character(len=*), intent(in) :: init_object
      integer(i4b) :: camb_module
      camb_module = camb_module2 ( (/init_object/) )
    end function camb_module

    function camb_module2(args)
#else
    program driver
#endif
#ifdef LEVELS
...
#ifdef LEVELS
        call module_startup('CAMB',args,.true.)
        call dmc_init(args)
...
#ifdef LEVELS
        call Reionization_ReadParams(P%Reion)
        call InitialPower_ReadParams(P%InitPower, P%WantTensors)
        call Recombination_ReadParams(P%Recomb)
#else
...
#ifdef LEVELS
        call Bispectrum_ReadParams(BispectrumParams, outroot)
#else
...
#ifdef LEVELS
            ! only to suppress a valgrind warning
...
100 stop 'Must give num_massive number of integer physical neutrinos for each eigenstate'
        end function camb_module2

end module camb_main
#else
    stop
