...
!FITS file has Cls without l(l+1)/twopi factors
  ...
  if (CP%OutputNormalization == 42) then
...
#ifdef LEVELS
  call dmc_create (out,Clsfile,"powerspectrum.LS_powspec_pol")
  call dmc_append_column(out,"TT",allcl(:,1))
  call dmc_append_column(out,"EE",allcl(:,2))
  call dmc_append_column(out,"BB",allcl(:,3))
  call dmc_append_column(out,"TE",allcl(:,4))
  call dmc_set_key(out,"CAMB_VERSION",version)
  call dmc_close(out)
#else
