...
#ifdef LEVELS
       subroutine Bispectrum_ReadParams(B, outroot)
          use IniFile
          Type(TBispectrumParams) :: B
          ...
          B%do_lensing_bispectrum = Ini_Read_Logical('do_lensing_bispectrum',.false.)
          B%do_primordial_bispectrum = Ini_Read_Logical('do_primordial_bispectrum',.false.)
          ...
          B%nfields = Ini_Read_Int('bispectrum_nfields',B%nfields)
          if (B%nfields /= 2 .and. B%nfields/=1) stop 'Bispectrum: nfields=1 for T only or 2 for polarization'
          B%do_parity_odd = Ini_Read_Logical('do_parity_odd',.false.)
          ...
          B%Slice_Base_L = Ini_Read_Int('bispectrum_slice_base_L',B%Slice_Base_L)
          if (B%Slice_Base_L>0) then
           B%ndelta = Ini_Read_Int('bispectrum_ndelta',B%ndelta)
           if (B%ndelta > max_bispectrum_deltas) stop 'Bispectrum : increase max_bispectrum_deltas'
           do i=1, B%ndelta
              B%deltas(i) = Ini_Read_Int_Array('bispectrum_delta', i)
           end do    
          ...
          B%DoFisher = Ini_Read_Logical('bispectrum_do_fisher',B%DoFisher)
          if (B%DoFisher) then
           B%FisherNoise = Ini_Read_Double('bispectrum_fisher_noise',B%FisherNoise)
           B%FisherNoisePol = Ini_Read_Double('bispectrum_fisher_noise_pol',B%FisherNoisePol )
           B%FisherNoiseFwhmArcmin = Ini_Read_Double('bispectrum_fisher_fwhm_arcmin',B%FisherNoiseFwhmArcmin)
          end if
          B%FullOutputFile = Ini_Read_String('bispectrum_full_output_file')
          if (B%FullOutputFile /='') then
           B%SparseFullOutput = Ini_Read_Logical('bispectrum_full_output_sparse')
          end if
          if (B%do_primordial_bispectrum) then
             B%export_alpha_beta = Ini_Read_Logical('bispectrum_export_alpha_beta',B%export_alpha_beta)
          end if
          end if
