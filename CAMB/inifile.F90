...
module IniFile
#ifdef LEVELS
 use planck_config, dum1=>fourpi, dum2=>pi
 use ls_paramfile_io
 use dmc_io
#endif
 implicit none
...
#ifdef LEVELS
  type (paramfile_handle), private, save :: params
  integer, parameter :: Ini_max_string_len = filenamelen
...
500 error=.true.
...
#ifdef LEVELS
   character(LEN=filenamelen) :: AValue
   AValue = parse_string(params,Key,'')
   if (AValue=='') then
     if (Ini_fail_on_not_found .or. &
         (present(NotFoundFail) .and. NotFoundFail)) then
  ...
  function Ini_Read_String_Default(Key, Default,AllowBlank) result(AValue)
   ...
   if (Ini_HasKey(Key)) then
    AValue = Ini_Read_String(Key, .false.)
 ...
 function Ini_Key_To_Arraykey(Key, index)  result(AValue)
...
#ifdef LEVELS
     if (present(NotFoundFail)) then
      AValue = Ini_Read_String(Ini_Key_To_Arraykey(Key,index),NotFoundFail)
     else
      AValue = Ini_Read_String(Ini_Key_To_Arraykey(Key,index))
     end if
#else
     if (present(NotFoundFail)) then
  ...
  function Ini_Read_Int_Array(Key, index, Default)
     ...
     if (present(Default)) then
      Ini_Read_Int_Array = Ini_Read_Int(Ini_Key_To_Arraykey(Key,index),Default)
     else
      Ini_Read_Int_Array = Ini_Read_Int(Ini_Key_To_Arraykey(Key,index))
     end if
#else
     if (present(Default)) then
...
#ifdef LEVELS
     if (present(Default)) then
       Ini_Read_Int = parse_int(params,Key,Default)
     else
       Ini_Read_Int = parse_int(params,Key)
     endif
#else
     if (present(Default)) then
...
#ifdef LEVELS
     if (present(Default)) then
       Ini_Read_Double = parse_double(params,Key,Default)
     else
       Ini_Read_Double = parse_double(params,Key)
     endif
#else
     if (present(Default)) then
...
#ifdef LEVELS
     if (present(Default)) then
      Ini_Read_Double_Array = Ini_Read_Double(Ini_Key_To_Arraykey(Key,index), Default)
     else
      Ini_Read_Double_Array = Ini_Read_Double(Ini_Key_To_Arraykey(Key,index))
     end if
#else
     if (present(Default)) then
...
#ifdef LEVELS
     if (present(Default)) then
       Ini_Read_Real = parse_real(params,Key,Default)
     else
       Ini_Read_Real = parse_real(params,Key)
     endif
#else
     if (present(Default)) then
...
#ifdef LEVELS
     if (present(Default)) then
      Ini_Read_Real_Array = Ini_Read_Real(Ini_Key_To_Arraykey(Key,index), Default)
     else
      Ini_Read_Real_Array = Ini_Read_Real(Ini_Key_To_Arraykey(Key,index))
     end if
#else
     if (present(Default)) then
...
#ifdef LEVELS
     if (present(Default)) then
       Ini_Read_Logical = parse_lgt(params,Key,Default)
     else
       Ini_Read_Logical = parse_lgt(params,Key)
     endif
#else
     if (present(Default)) then
