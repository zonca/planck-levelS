...
#ifdef LEVELS
       subroutine InitialPower_ReadParams(InitPower, WantTensors)
#else
       subroutine InitialPower_ReadParams(InitPower, Ini, WantTensors)
          ...
          integer i
...
#ifdef LEVELS
           InitPower%k_0_scalar = Ini_Read_Double('pivot_scalar',InitPower%k_0_scalar)
           InitPower%k_0_tensor = Ini_Read_Double('pivot_tensor',InitPower%k_0_tensor)
           ...
           do i=1, InitPower%nn
              ...
              InitPower%an(i) = Ini_Read_Double_Array('scalar_spectral_index', i)
              InitPower%n_run(i) = Ini_Read_Double_Array('scalar_nrun',i,0._dl)
              
              if (WantTensors) then
                 InitPower%ant(i) = Ini_Read_Double_Array('tensor_spectral_index',i)
                 InitPower%rat(i) = Ini_Read_Double_Array('initial_ratio',i)
              end if              

              InitPower%ScalarPowerAmp(i) = Ini_Read_Double_Array('scalar_amp',i,1._dl)
