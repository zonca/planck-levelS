...
#ifdef LEVELS
 subroutine Reionization_ReadParams(Reion)
  use IniFile
  Type(ReionizationParams) :: Reion

   Reion%Reionization = Ini_Read_Logical('reionization')
   if (Reion%Reionization) then
   
    Reion%use_optical_depth = Ini_Read_Logical('re_use_optical_depth')
  
    if (Reion%use_optical_depth) then
              Reion%optical_depth = Ini_Read_Double('re_optical_depth')
    else 
              Reion%redshift = Ini_Read_Double('re_redshift')
    end if 

    Reion%delta_redshift = Ini_Read_Double('re_delta_redshift', 0.5_dl) !default similar to CMBFAST original
    Reion%fraction = Ini_Read_Double('re_ionization_frac',Reionization_DefFraction)
